...
-module(emqx_gw_hook_actions).

-include_lib("emqx/include/emqx.hrl").
-include_lib("emqx/include/logger.hrl").

-define(RESOURCE_TYPE_GWHOOK, 'gw_hook').
...
-resource_type(#{name => ?RESOURCE_TYPE_GWHOOK,
                 ...
                 title => #{en => <<"GwHook">>,
                            zh => <<"GwHook"/utf8>>},
                 description => #{en => <<"GwHook">>,
                                  zh => <<"GwHook"/utf8>>}
                }).

-rule_action(#{name => data_to_gwserver,
               for => '$any',
               create => on_action_create_data_to_gwserver,
               params => ?ACTION_DATA_SPEC,
               types => [?RESOURCE_TYPE_GWHOOK],
               title => #{en => <<"Data to gw Server">>,
                          zh => <<"发送数据到 gw 服务"/utf8>>},
               description => #{en => <<"Forward Messages to gw Server">>,
                                zh => <<"将数据转发给 gw 服务"/utf8>>}
              }).
...
-export([ on_action_create_data_to_gwserver/2
        ]).
    ...
    case emqx_rule_utils:http_connectivity(Url) of
        ...
        {error, Reason} ->
            ?LOG(error, "Initiate Resource ~p failed, ResId: ~p, ~0p",
                [?RESOURCE_TYPE_GWHOOK, ResId, Reason]),
...
%% An action that forwards publish messages to a remote gw server.
-spec(on_action_create_data_to_gwserver(Id::binary(), #{url() := string()}) -> action_fun()).
on_action_create_data_to_gwserver(_Id, Params) ->
...
http_request(Url, Headers, Method, Params) ->
    logger:debug("[GwHook Action] ~s to ~s, headers: ~s, body: ~p", [Method, Url, Headers, Params]),
    case http_request(Method, ?JSON_REQ(Url, Headers, jsx:encode(Params)),
                      [{timeout, 5000}], [], 0) of
        {ok, _} -> ok;
        {error, Reason} ->
            logger:error("[GwHook Action] HTTP request error: ~p", [Reason])
    end.
